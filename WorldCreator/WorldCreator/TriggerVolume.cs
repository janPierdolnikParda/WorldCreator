...
namespace WorldCreator
{
    public class TriggerVolume : GameObject
    {
        ...
        public delegate void CharacterEnteredHandler(TriggerVolume sender);
        public delegate void CharacterLeftHandler(TriggerVolume sender);
            ...
            for (int i = ObjectsInside.Count - 1; i >= 0; i--)
            {
                ...
                switch (info.State)
                {
                    case ObjectState.Unknown:
                        if (info.GameObject.Exists && OnCharacterLeft != null)
                            OnCharacterLeft(this);
                        ObjectsInside.RemoveAt(i);
                        break;

                    case ObjectState.New:
                        if (info.GameObject.Exists && OnCharacterEntered != null)
